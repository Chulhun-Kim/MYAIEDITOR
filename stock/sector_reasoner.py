...
try:
    from stock.theme_graph import (
        build_theme_graph,
        summarize_theme_graph,
        theme_graph_to_markdown,
    )
except Exception:
    def build_theme_graph(news_items=None, indicators=None, extra_text="", min_score=18.0, limit=8):
        return []

    def summarize_theme_graph(theme_nodes, limit=3):
        return "뚜렷하게 부각된 테마 그래프가 감지되지 않았습니다."

    def theme_graph_to_markdown(theme_nodes, limit=5):
...
class ThemeResult:
    ...
    # v2.1 Theme Graph 연결 필드
    parent_theme: str = ""
    child_themes: List[str] = None
    graph_story: str = ""

    def to_dict(self) -> Dict[str, Any]:
        d = asdict(self)
        if d.get("child_themes") is None:
            d["child_themes"] = []
        return d
...
def analyze_themes(
    ...
    # v2.1 Theme Graph: 뉴스 전체에서 상위 테마 그래프를 먼저 만든다.
    graph_nodes = build_theme_graph(
        news_items=news_items,
        extra_text=text,
        min_score=18.0,
        limit=8,
    )
    graph_by_theme = {str(g.get("theme", "")): g for g in graph_nodes if isinstance(g, dict)}
    ...
    for rule in THEME_RULES.values():
        ...
        graph_node = graph_by_theme.get(rule.name, {}) or {}
        if graph_node:
            score = min(score + 6, 100)

        graph_story = str(graph_node.get("story", "") or "")
        parent_theme = str(graph_node.get("parent", "") or "")
        child_themes = list(graph_node.get("children", []) or [])

        reasons = _top_reasons(rule)
        if graph_story and graph_story not in reasons:
            reasons = reasons + [graph_story]
        ...
        results.append(
            ThemeResult(
                ...
                child_themes=child_themes[:6],
                graph_story=graph_story,
            )
        )
...
def analyze_theme_graph(
    ...
    """
    theme_graph.py의 build_theme_graph()를 theme_reasoner.py 쪽 Public API로 노출한다.
    다른 모듈은 theme_reasoner만 import해도 Theme Graph를 사용할 수 있다.
    """
    return build_theme_graph(
        news_items=news_items,
...
def summarize_theme_graph_from_news(
    ...
    """
    뉴스 기반 Theme Graph 요약문.
    Workspace / Market Story / AI Prompt에서 바로 사용할 수 있다.
    """
    graph = analyze_theme_graph(
        news_items=news_items,
        indicators=indicators,
        limit=max(limit, 3),
    )
    return summarize_theme_graph(graph, limit=limit)


def theme_graph_markdown_from_news(
    news_items: Optional[List[Any]] = None,
    indicators: Optional[List[Dict[str, Any]]] = None,
    limit: int = 5,
) -> str:
    """
    뉴스 기반 Theme Graph Markdown 출력.
    """
    graph = analyze_theme_graph(
        news_items=news_items,
        indicators=indicators,
    ...
    return theme_graph_to_markdown(graph, limit=limit)
