...
# - NewsAPI = "검색 기반 수집기"로 단순화 (Top Headlines 제거)
# - q(검색어) 필수
# - 기간 프리셋(24h/3d/7d) + 캘린더(date_input) 지원
# - session_state 캐시 유지(미리보기/반영 버튼용)
...
def render_newsapi_panel(newsapi_key: str) -> Tuple[Optional[str], List[Dict]]:
    """
    반환:
      - workspace_text_or_none: Workspace에 추가할 텍스트(없으면 None)
      - buffer_items: Buffer에 추가할 항목(list[dict]) (없으면 [])
    """
    st.subheader("🧩 NewsAPI 수집 (Everything: 검색 전용)")
    _init_cache()

    if not (newsapi_key or "").strip():
        st.warning("NEWSAPI_KEY가 없습니다. .env에 NEWSAPI_KEY를 설정하세요.")
    ...
    with row1[1]:
        language = st.selectbox("언어", ["", "ko", "en", "ja", "zh", "de", "fr"], index=1, key="newsapi_language")
    with row1[2]:
        sort_by = st.selectbox("정렬", ["relevancy", "publishedAt", "popularity"], index=0, key="newsapi_sort_by")
    with row1[3]:
        limit = st.number_input("개수", min_value=5, max_value=50, value=10, step=5, key="newsapi_limit")

    row2 = st.columns([1, 1, 2])
    with row2[0]:
        do_fetch = st.button("NewsAPI 불러오기", use_container_width=True, key="newsapi_fetch")
    with row2[1]:
        if st.button("캐시 비우기", use_container_width=True, key="newsapi_clear_cache"):
            clear_newsapi_cache()
    ...
    with st.expander("기간/도메인/소스(옵션)"):
        ...
        cA, cB = st.columns(2)
        with cA:
            from_dt = st.date_input("from (캘린더)", value=st.session_state.newsapi_from_dt, key="newsapi_from_dt")
            domains = st.text_input(
                "domains (콤마구분)",
                value="",
                key="newsapi_domains",
                help="예: reuters.com, apnews.com (특정 사이트만 검색)",
            )
        with cB:
            to_dt = st.date_input("to (캘린더)", value=st.session_state.newsapi_to_dt, key="newsapi_to_dt")
            sources = st.text_input(
                "sources (콤마구분)",
                value="",
                key="newsapi_sources",
                help="NewsAPI 소스 ID(권한/플랜에 따라 제한). 보통 domains를 권장",
            )
    ...
    try:
        ...
        query_info: Dict = {"mode": "everything", "q": q.strip(), "sortBy": sort_by}
        ...
        items = normalize_articles(payload, query_info=query_info, limit=int(limit))
        if not items:
        ...
        st.session_state.newsapi_last_info = f"NewsAPI/Everything - {len(items)}건 (q={q.strip()} / 수집: {items[0].fetched_at})"
